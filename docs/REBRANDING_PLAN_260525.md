...
Redesign the visible GJC terminal, export, and documentation surfaces around a coherent red-claw gajae-code identity while preserving compatibility boundaries.

The default-visible product should read as **gajae-code / GJC**, not OMP/Oh My Pi or a generic inherited terminal skin. Red-claw becomes the default dark visual direction for users without an explicit override. Session exports and README screenshots should show the same brand direction, while exported transcript content remains neutral and readable.

## Principles

1. **GJC-first visible identity** — Default-visible UI should present gajae-code/red-claw as the current product identity.
2. **Compatibility preservation** — Keep `gjc`, `gjc-stats`, `gjc-swarm`, `@gajae-code/*`, legacy runtime roots/env aliases, and explicit attribution/history.
...
- Status-line identity cutover away from default-visible Pi/OMP/powerline styling.
...
- Static scans and tests for current-product brand leaks, compatibility names, theme defaults, fallback readability, and export branding.

### Out of scope

- Renaming `gjc`, `gjc-stats`, `gjc-swarm`, or `@gajae-code/*` package surfaces.
- Removing legacy runtime roots, env aliases, compatibility internals, migration notes, generated/vendor content, or attribution/history solely because they mention OMP/Pi/OMX.
- Copying OpenAI Codex, SST/opencode, Claude Code, or Oh My Pi visuals verbatim.
...
- Search active visible UI/docs/export surfaces for old-brand and inherited UI identity markers: `omp`, `oh-my-pi`, `omx`, `pi`, `powerline`, and generic export labels.
- Classify hits as current product identity, explicit user opt-in setting labels, compatibility internals, attribution/history/migration notes, or generated/vendor content.
- Build or update verification gates so current-product visible leaks fail, but compatibility and attribution do not.
...
- Remove Pi from bundled default-visible status presets or replace it with compact GJC/claw identity.
- Preserve legacy segment/symbol compatibility only as explicit opt-in or internal alias behavior.
...
### Phase 4 — Coherent TUI component pass

Use existing theme tokens rather than a new UI framework abstraction.

- Apply shell/ink backgrounds, coral/claw accents, compact borders, and lower-noise hierarchy across visible components.
...
- Keep message bodies, code blocks, tool output, system prompts, and transcript content neutral and high contrast.
...
- Run final code review and resolve blockers before considering the implementation complete.
...
- [ ] Default-visible status-line identity no longer leads with Pi/OMP-style branding.
- [ ] Default-visible status separators no longer use powerline-style styling unless explicitly opted in.
- [ ] Visible TUI components share one coherent GJC language across welcome, status line, footer hints, message frames, tool execution cards, ask/approval cards, selectors/settings, and todo/plan surfaces.
- [ ] Static scans of active UI/docs/export surfaces do not present OMP/OMX/Pi as current product identity; compatibility internals, attribution/history, generated/vendor content, and migration notes remain allowlisted.
...
- [ ] Focused verification covers default theme, visible brand allowlist, export branding, and preserved compatibility names.
...
3. Render status line at normal and narrow widths for default, compact, full, Nerd, ASCII, and preserved custom settings.
...
7. Inspect README screenshots/alt text and compare them against the generated full-session export direction.
...
- **Visible Pi/OMP removal breaks legacy configs** — Keep compatibility aliases internally or opt-in, while removing current-product default visibility.
...
This plan is approved for tracking. Implementation still requires normal code review and verification before completion.
